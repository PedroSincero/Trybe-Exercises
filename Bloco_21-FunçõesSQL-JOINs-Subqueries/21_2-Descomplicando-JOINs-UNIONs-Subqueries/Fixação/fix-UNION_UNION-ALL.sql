...
-- 2: Monte uma query que una os resultados das tabelas customer e actor , exibindo os nomes que contém a palavra "tracy" na tabela customer e os que contém "je" na tabela actor . Exiba apenas os resultados únicos.
SELECT 
    first_name
FROM
    sakila.customer
...
-- 3: Monte uma query que exiba a união dos 5 últimos nomes da tabela actor , o primeiro nome da tabela staff e 5 nomes a partir da 15ª posição da tabela customer . Não permita que dados repetidos sejam exibidos. Ordene os resultados em ordem alfabética.
(SELECT 
    first_name
FROM
    sakila.actor
